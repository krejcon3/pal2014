...
public class Main {
	...
	public static boolean[] toUse;
	public static int start;
	public static double[] bestWays;

	/**
	 * zničit hashset -> nahradit za pole
	 */
...
//		System.setIn(new FileInputStream("./src/test/pub01.in"));
		...
		for (int i = 0; i < count; i++) {
			start = i;
			toUse[i] = false;
			work(toUse, new int[count], i, 0, 0);
			toUse[i] = true;
			if (bestWays[bestWays.length - i - 1] > 0) {
				System.out.println((int)Math.ceil(bestWays[bestWays.length - i - 1]));
				break;
			}
		}
	}

	public static void work(boolean[] array, int[] used, int site, double length, int depth) {
		used[depth] = site;
		int tStart;
		int tEnd;
		boolean crossed;
		for (int index = 0; index < array.length; index++) {
			...
			crossed = false;
			double toNext = getDistance(sites[site], sites[index]);
			double toHome = getDistance(sites[index], sites[start]);
			if (length + toNext + toHome > limit) {
				continue;
			}
			if (bestWays[bestWays.length - start - 1] > 0 && length + toNext + toHome > bestWays[bestWays.length - start - 1]) {
				continue;
			}
			tEnd = -1;
			for (int i = 0; i < depth + 1; i++) {
				...
				if (intersectionFounded(sites[site], sites[index], sites[tStart], sites[tEnd])) {
					crossed = true;
					break;
				}
			}
			if (!crossed && depth < count - 1) {
				toUse[index] = false;
				work(array, used, index, length + toNext, depth + 1);
				toUse[index] = true;
			}
		}
		tEnd = -1;
